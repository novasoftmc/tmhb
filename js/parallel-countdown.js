      ...
      if (data.timers && data.timers.length > 0) {
        timers = data.timers.map((t, i) => {
          ...
          return {
            ...
            soundSettings: t.soundSettings || { useGlobal: true, soundType: null, volume: null },
  ...
  function createTimerBox(timerData, index) {
    ...
    box.innerHTML = `
      ...
      index + 1
                    ...
                    <div style="position: absolute; right: 10px; top: 65%; transform: translateY(-50%); display: flex; flex-direction: column; align-items: center; gap: 6px;">
      ...
      timerData.beepAt === 10 ? "checked" : ""
                        ...
                        <button class="advanced-sound-btn advanced-sound-icon" data-timer-index="${index}" title="Click to change sound" style="margin-top: -2px;">${timerData.soundSettings?.useGlobal === false && timerData.soundSettings?.soundType ? sharedSound.SOUND_OPTIONS.find(o => o.value === timerData.soundSettings.soundType)?.label + ' ▼' : 'Timer ' + (index + 1) + ' Sound ▼'}</button>
                    </div>
      ...
      try {
        ...
        if (isVeryEnd) {
          ...
          if (beepAt > 0) {
            ...
            const snd = getTimerSoundSettings(timerIndex);
...
function playReminderSound(duration, timerIndex) {
    ...
    if (timer.remainingSeconds > 0) {
      ...
      if (reminders.custom && reminders.custom.length > 0) {
        reminders.custom.forEach((rem, remIdx) => {
          ...
          if (timer.remainingSeconds === remSec) {
            if (rem.sound !== false) {
              playReminderSound(duration, index);
            }
            if (rem.flash !== false) {
      ...
      if (reminders.every) {
        ...
        if (every > 0 && timer.remainingSeconds % every === 0 && timer.remainingSeconds > 0) {
          if (reminders.every.sound !== false) {
            playReminderSound(duration, index);
          }
      ...
      if (beepAt > 0 && timer.remainingSeconds <= beepAt && timer.remainingSeconds > 0 && state.getSoundEnabled()) {
        const snd = getTimerSoundSettings(index);
        sharedSound.playSound(snd.soundType, snd.volume);
      }
...
function renderAllTimers() {
  ...
  // Update timers count input to match actual timer count
  const timersCountInput = document.getElementById("timers-count-input");
  if (timersCountInput) {
    timersCountInput.value = timerCount;
  }
}
