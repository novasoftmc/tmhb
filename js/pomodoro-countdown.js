...
const timerLogic = (function () {
  ...
  function nextSequenceItem() {
    ...
    if (nextIndex < sequence.length) {
      ...
      console.log('=== Timer sequence finished ===');
      console.log('Current sequence item:', state.getCurrentSequenceItem());
      console.log('Completed item:', completedItem);
      
      if (completedItem && completedItem.type === "timer") {
        console.log('Duration in seconds:', completedItem.duration);
        const durationMinutes = completedItem.duration / 60; // Keep as decimal, don't round to 0
        console.log('Duration in minutes:', durationMinutes);
        statsManager.recordPomodoro(durationMinutes);
      } else {
        console.log('NOT recording - not a timer or no completed item');
      }
...
const statsManager = {
  ...
  getStats() {
    ...
    return {
      ...
      streak: 0,
      lastDate: null,
      totalTimeToday: 0, // in minutes
      breaksToday: 0,
      breakTimeToday: 0, // in minutes
      history: [], // {date, count, duration}
      completed: 0,
      started: 0,
      dailyData: {} // {date: {pomodoros, minutes}}
    };
  },
  ...
  checkNewDay() {
    ...
    if (stats.lastDate !== today) {
      ...
      if (stats.lastDate && stats.today > 0) {
        stats.history.push({
          date: stats.lastDate,
          count: stats.today,
          duration: stats.totalTimeToday
        });
  ...
  recordPomodoro(durationMinutes) {
    console.log('=== recordPomodoro called ===');
    console.log('Duration received:', durationMinutes);
    
    let stats = this.checkNewDay();
    ...
    stats.month++;
    stats.completed++;
    stats.totalTimeToday += durationMinutes;
    
    const today = new Date().toDateString();
    if (!stats.dailyData[today]) {
      stats.dailyData[today] = { pomodoros: 0, minutes: 0 };
    }
    stats.dailyData[today].pomodoros++;
    stats.dailyData[today].minutes += durationMinutes;
    
    console.log('Stats after update:', JSON.stringify(stats));
    console.log('Total time today:', stats.totalTimeToday);
    
    this.saveStats(stats);
    this.updateStatsDisplay();
  },

  // Record started pomodoro
  recordStart() {
  ...
  // Record break
  recordBreak(durationMinutes) {
    let stats = this.checkNewDay();
    stats.breaksToday++;
    stats.breakTimeToday += durationMinutes;
    this.saveStats(stats);
    this.updateStatsDisplay();
  },
  ...
  updateStatsDisplay() {
    ...
    const hours = Math.floor(stats.totalTimeToday / 60);
    const mins = Math.round(stats.totalTimeToday % 60);
    document.getElementById('stat-time-today').textContent = `${hours}h ${mins}m`;
    
    // Calculate averages
    const avgPerDay = stats.history.length > 0 
      ? (stats.history.reduce((sum, day) => sum + day.count, 0) / stats.history.length).toFixed(1)
      : '0.0';
    document.getElementById('stat-avg-day').textContent = avgPerDay;
    
    const avgLength = stats.completed > 0
      ? Math.round((stats.totalTimeToday + stats.history.reduce((sum, day) => sum + day.duration, 0)) / (stats.completed || 1))
      : 25;
    document.getElementById('stat-avg-length').textContent = `${avgLength}m`;
    ...
    document.getElementById('stat-breaks-today').textContent = stats.breaksToday;
    document.getElementById('stat-break-time').textContent = `${stats.breakTimeToday}m`;
  ...
  drawChart(stats) {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayData = stats.dailyData[dateStr] || { pomodoros: 0, minutes: 0 };
      last7Days.push({
        label: date.toLocaleDateString('en-US', { weekday: 'short' }),
        value: Math.round(dayData.minutes / 60 * 10) / 10 // hours with 1 decimal
      });
    }
    ...
    last7Days.forEach((day, index) => {
      ...
      ctx.fillText(day.label, x + barWidth * 0.3, height - 10);
      ctx.fillText(day.value + 'h', x + barWidth * 0.3, y - 5);
    });
  },
